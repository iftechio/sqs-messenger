...
Consumer.prototype._handleSqsResponse = function (err, response) {
  ...
  if (response && response.Messages && response.Messages.length) {
    ...
    this.processingMessagesPromise = Promise.map(response.Messages, message => this._processMessage(message))
...
Consumer.prototype.shutdown = function (timeout) {
  ...
  return this.processingMessagesPromise.timeout(timeout).catch(err => {
    debug('shutdown timeout', timeout, err)
  })
}
